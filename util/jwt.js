...
const { updateStatus } = new UpdateClass.UpdateClass();
...
async function generateToken(params) {
  try {
    ...
    const xAccessToken = jwt.sign(tokenParams, JWT_CONFIG.secret);
...
async function validateTokenExpiry(params) {
  return new Promise((resolve, reject) => {
    try {
      jwt.verify(
        ...
        (error, verifiedData) => {
          if (verifiedData) {
            ...
            console.error(
              `JWT Error :: api/util/api_token.js : verifyToken() :: ${error}`
            );
            resolve(false);
          }
        }
      ...
      console.error(
        " API :: UTIL :: API TOKEN :: verifyUserDeviceAndToken() ::=>  ",
        error
      );
      reject(error);
    }
