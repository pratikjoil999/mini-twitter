...
async function saveService(params) {
  try {
    let finalReponse = {};
    const alreadyExist =
      params.email != "" ? await countEmail(params) : await countMobile(params);
    if (alreadyExist === 0) {
  ...
  try {
    ...
    if (userById) {
      return (finalResponse = {
        data: userById,
        message: staticMsg.user.findById[200],
      });
    } else {
      return (finalResponse = {
        data: [],
        message: staticMsg.user.findById[404],
      });
    }
  } catch (error) {
    console.error(
      "ERROR : service || v1 || user || findByIdService() : ",
      error
    );
    throw error;
  }
...
async function logoutUserService(params) {
  try {
    const x_access_token = await JWT.generateToken(params);
